...
const DataTree = ({
    ...
    const nodeRenderer = ({
        ...
        const handleClick = (e) => {
            let actionElement = e.target.closest('[data-add], [data-addcopy], [data-remove], [data-open], [data-close]');
            let isAction = false;
            ...
            if (actionElement) {
                ...
                else if (actionElement.hasAttribute('data-addcopy')) { isAction = true; actionType = 'addcopy'; }
            ...
            if (actionType === 'add' && attrs['data-add']) {
                ...
                if ([DATA_TYPES.NUMBER, DATA_TYPES.STRING].includes(ref)) {
                    ...
                    if (currentItemSchema.hasOwnProperty('enum') && Object.keys(currentItemSchema).length === 1) {
                        ...
                        // Corrected string concatenation for xpath
            ...
            if (actionType === 'addcopy' && attrs['data-addcopy']) {
        ...
        return (
            <div 
                ...
                // Assign handleClick to the main div wrapper. 
                // Clicks on arrows or specific action buttons inside Component will be handled by handleClick logic.
                // Clicks on the general body of the Component will also be routed to handleClick.
                onClick={handleClick} 
            >
                <Component 
                    data={{
                    ...
                    isOpen={isExpanded}
                    // Pass handleClick to child Component (HeaderField/Node) to be used for its interactive elements (arrows, option buttons)
                    // The e.target in handleClick will determine what was actually clicked.
                    onClick={handleClick} 
                />
            </div>
        );
